  ...
  const fetchWishlist = async () => {
    try {
      ...
      const totalP = Math.max(
        1,
        Number(pagination.totalPages ?? payload.totalPages ?? 1) || 1,
      );
      const totalI = Number(
        payload.totalItems ?? pagination.total ?? wishlistData.length,
      );
        ...
        <div className="flex-1 max-w-7xl mx-auto w-full px-6 py-20">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="h-[450px] bg-white/50 rounded-[2.5rem] animate-pulse border-2 border-outline-variant/5"
              />
            ))}
          </div>
        </div>
        ...
        {wishlistItems.length === 0 ? (
          ...
          >
            ...
            <p className="text-on-surface-variant font-medium text-lg mb-10 max-w-md mx-auto leading-relaxed">
              Bạn Artie chưa thấy bé thả tim khóa học nào hết! Hãy cùng khám phá
              những bài học vẽ siêu vui nhé!
            </p>
            <button
              ...
              <AnimatePresence mode="popLayout">
                {wishlistItems.map((item, idx) => {
                  ...
                  const instructorName =
                    p.instructor?.fullname || "Họa sĩ ArtKids";
                      ...
                      >
                        ...
                        <div className="p-8 flex flex-col flex-1">
                          ...
                          <div className="flex items-center gap-5 text-on-surface-variant/60 font-bold text-[11px] mb-8">
                            ...
                            <div className="flex items-center gap-1.5">
                              <span className="material-symbols-outlined text-base text-secondary/40">
                                video_library
                              </span>
                              {isCombo
                          ...
                          <div className="mt-auto pt-6 border-t-2 border-dashed border-outline-variant/10">
                            <div className="flex items-center justify-between mb-6">
                              ...
                              <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                                <span className="material-symbols-outlined text-primary fill-primary">
                                  favorite
                                </span>
                              </div>
                            </div>
